...
class LeroySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        links = response.css('div.largeCard').xpath('a/@href')
        #TODO: unique
        for link in links[:1]:
...
def parse_leroy(url, user_id):
    ...
    process.crawl(LeroySpider, url=url, path=path)
    process.start()
    process.stop()
...
if __name__ == "__main__":
    url = 'https://spb.leroymerlin.ru/catalogue/bytovaya-himiya/sredstva-dlya-kovrov-i-mebeli/'
    user_id = '511002883'
    parse_leroy(url, user_id)
